...
// This is idempotent â€” it skips creation if the data already exists.
...
func seedRoles(db *gorm.DB) {
	roles := []struct {
		...
		{
			Name:        "Administrator",
			Description: "Full system access â€” all privileges granted",
			Privileges:  domain.AllPrivileges(),
		},
		{
			Name:        "Manager",
			Description: "Operational management â€” approval, analytics, and oversight",
			Privileges: []string{
		...
		{
			Name:        "Technician",
			Description: "Field technician â€” maintenance and asset operations",
			Privileges: []string{
		...
		{
			Name:        "Operator",
			Description: "Operator â€” perform maintenance tasks",
			Privileges: []string{
		...
		{
			Name:        "Planner",
			Description: "Planner â€” maintenance planning and scheduling",
			Privileges: []string{
				domain.PrivilegeAssetView,
				domain.PrivilegeWorkOrderView, domain.PrivilegeWorkOrderCreate, domain.PrivilegeWorkOrderUpdate,
				domain.PrivilegeMaintenanceView, domain.PrivilegeMaintenancePlanCreate,
		...
		{
			Name:        "Auditor",
			Description: "Auditor â€” perform audit tasks",
			Privileges: []string{
				domain.PrivilegeAssetView,
...
func seedAdminUser(db *gorm.DB) {
	...
	if adminPassword == "" {
		...
		log.Println("Seeder: WARNING â€” Using default admin password. Set DEFAULT_ADMIN_PASSWORD env var in production!")
	}
