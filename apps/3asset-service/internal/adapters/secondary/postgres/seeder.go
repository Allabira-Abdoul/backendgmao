...
func Seed(db *gorm.DB) {
	...
	// Seed some consumables
	consOil := createConsumable(db, "ENGINE OIL 15W40", "LUBRICANTS", "Gallons")
	createConsumableStock(db, consOil.ID, "PARK GSE", 50)
	
	consScrews := createConsumable(db, "M6 BOLT", "HARDWARE", "Box")
	createConsumableStock(db, consScrews.ID, "PARK GSE", 20)
...
func createConsumable(db *gorm.DB, name, category, uom string) domain.Consumable {
	var c domain.Consumable
	db.Where(domain.Consumable{Name: name}).
		Assign(domain.Consumable{Category: category, UnitOfMeasure: uom, TotalStock: 0}).
		FirstOrCreate(&c)
	return c
}

func createConsumableStock(db *gorm.DB, consumableID uuid.UUID, location string, qty int) domain.ConsumableLocationStock {
	var s domain.ConsumableLocationStock
	db.Where(domain.ConsumableLocationStock{ConsumableID: consumableID, Location: location}).
		Assign(domain.ConsumableLocationStock{Quantity: qty}).
		FirstOrCreate(&s)

	// Update Total Stock
	var c domain.Consumable
	if err := db.First(&c, "id = ?", consumableID).Error; err == nil {
		c.TotalStock += qty
		db.Save(&c)
	}
