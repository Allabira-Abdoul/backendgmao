...
func (r *assetRepository) CreateConsumable(ctx context.Context, consumable *domain.Consumable) error {
	return r.db.WithContext(ctx).Create(consumable).Error
}

func (r *assetRepository) GetConsumables(ctx context.Context) ([]domain.Consumable, error) {
	var consumables []domain.Consumable
	err := r.db.WithContext(ctx).Preload("Suppliers").Preload("Suppliers.Supplier").Preload("LocationStock").Find(&consumables).Error
	return consumables, err
}

func (r *assetRepository) GetConsumableByID(ctx context.Context, id uuid.UUID) (*domain.Consumable, error) {
	var consumable domain.Consumable
	err := r.db.WithContext(ctx).Preload("Suppliers").Preload("Suppliers.Supplier").Preload("LocationStock").First(&consumable, "id = ?", id).Error
	return &consumable, err
}

func (r *assetRepository) GetConsumableStock(ctx context.Context, consumableID uuid.UUID, location string) (*domain.ConsumableLocationStock, error) {
	var stock domain.ConsumableLocationStock
	err := r.db.WithContext(ctx).Where("consumable_id = ? AND location = ?", consumableID, location).First(&stock).Error
	return &stock, err
}

func (r *assetRepository) UpdateConsumableStock(ctx context.Context, stock *domain.ConsumableLocationStock) error {
	return r.db.WithContext(ctx).Save(stock).Error
}

func (r *assetRepository) UpdateConsumable(ctx context.Context, consumable *domain.Consumable) error {
	return r.db.WithContext(ctx).Save(consumable).Error
}

func (r *assetRepository) CreateConsumableConsumptionLog(ctx context.Context, log *domain.ConsumableConsumptionLog) error {
	return r.db.WithContext(ctx).Create(log).Error
}
