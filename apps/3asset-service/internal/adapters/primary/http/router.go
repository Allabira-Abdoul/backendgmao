...
func RegisterRoutes(
	...
	authenticated.Use(middleware.RequireAuth(jwtManager))
	{
		...
		consumables := authenticated.Group("/consumables")
		{
			consumables.POST("", middleware.RequirePrivilege("ASSET_CREATE"), assetHandler.CreateConsumable)
			consumables.GET("", middleware.RequirePrivilege("ASSET_VIEW"), assetHandler.GetConsumables)
			consumables.GET("/:id", middleware.RequirePrivilege("ASSET_VIEW"), assetHandler.GetConsumableByID)
			consumables.POST("/:id/stock", middleware.RequirePrivilege("ASSET_UPDATE"), assetHandler.AddConsumableStock)
		}
		...
		actions := authenticated.Group("/actions")
		{
			...
			actions.POST("/consume-consumable", middleware.RequirePrivilege("ASSET_UPDATE"), assetHandler.ConsumeConsumable)
